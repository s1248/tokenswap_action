...
contract('BActions', async (accounts) => {
    ...
    describe('Private pool controls', () => {
        ...
        it('rebind add new token', async () => {
            ...
            await userProxy.methods['execute(address,bytes)'](KACTIONS, inputData);

            const kpool = await BPool.at(POOL);
            const currentTokens = await kpool.getCurrentTokens();
        ...
        it('rebind decrease balance token', async () => {
            ...
            const kpool = await BPool.at(POOL);
            const poolZrxBalance = await kpool.getBalance(ZRX);
        ...
        it('fails other user interaction', async () => {
            ...
            await truffleAssert.reverts(
                userProxy.methods['execute(address,bytes)'](KACTIONS, inputData, { from: user1 }),
            ...
            await truffleAssert.reverts(
                kpool.rebind(ZRX, toWei('5'), toWei('5')),
                'ERR_NOT_CONTROLLER',
            );

            await truffleAssert.reverts(
                kpool.rebind(ZRX, toWei('5'), toWei('5'), { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
        });

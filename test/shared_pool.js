...
contract('BActions', async (accounts) => {
    ...
    describe('Shared pool', () => {
        ...
        it('pushes correct amount of pool shares to creator', async () => {
            const kpool = await BPool.at(POOL);
            const shareBalance = await kpool.balanceOf(creator);
        ...
        it('is finalized', async () => {
            const kpool = await BPool.at(POOL);
            const isFinalized = await kpool.isFinalized.call();
        ...
        it('has correct tokens', async () => {
            const kpool = await BPool.at(POOL);
            const tokens = await kpool.getFinalTokens.call();
        ...
        it('has correct amounts', async () => {
            ...
            const daiBalance = await kpool.getBalance(DAI);
            const mkrBalance = await kpool.getBalance(MKR);
            const wethBalance = await kpool.getBalance(WETH);
        ...
        it('allows join from user', async () => {
            ...
            const initialShareBalance = await kpool.balanceOf(user);

            const functionCall = web3.eth.abi.encodeFunctionCall(joinInterface, params);
            await userProxy.methods['execute(address,bytes)'](KACTIONS, functionCall, {
                from: user,
            });
            ...
            const shareBalance = await kpool.balanceOf(user);

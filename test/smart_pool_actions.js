...
contract('BActions', async (accounts) => {
    ...
    describe('Smart pool actions', () => {
        ...
        beforeEach(async () => {
            kactions = await BActions.deployed();
            KACTIONS = kactions.address;
            ...
            const poolAddress = await creatorProxy.methods['execute(address,bytes)'].call(
                KACTIONS, functionCall,
            );
            POOL = `0x${poolAddress.slice(-40)}`;

            await creatorProxy.methods['execute(address,bytes)'](KACTIONS, functionCall);
        ...
        it('does not allow joining', async () => {
            ...
            await truffleAssert.fails(
                creatorProxy.methods['execute(address,bytes)'](KACTIONS, functionCall),
        ...
        it('does not allow changing params by user', async () => {
            ...
            await truffleAssert.fails(
                userProxy.methods['execute(address,bytes)'](KACTIONS, functionCall, { from: user }),
                truffleAssert.ErrorType.REVERT,
                'ERR_NOT_CONTROLLER',
            );
        ...
        it('allows toggling public swap', async () => {
            const kpool = await BPool.at(UNDERLYING_POOL);

            const initialPublicSwap = await kpool.isPublicSwap();
            ...
            const publicSwap = await kpool.isPublicSwap();
        ...
        it('allows changing swap fee', async () => {
            ...
            const initialSwapFee = await kpool.getSwapFee();
            ...
            const swapFee = await kpool.getSwapFee();
        ...
        it('allows increasing token weight', async () => {
            ...
            const initialWeight = await kpool.getDenormalizedWeight(MKR);
            ...
            const weight = await kpool.getDenormalizedWeight(MKR);
        ...
        it('allows decreasing token weight', async () => {
            ...
            const initialWeight = await kpool.getDenormalizedWeight(DAI);
            ...
            const weight = await kpool.getDenormalizedWeight(DAI);
        ...
        it('allows updating token weights gradually', async () => {
            ...
            const initialDaiWeight = await kpool.getDenormalizedWeight(DAI);
            const initialMkrWeight = await kpool.getDenormalizedWeight(MKR);
            const initialWethWeight = await kpool.getDenormalizedWeight(WETH);
            ...
            const daiWeight = await kpool.getDenormalizedWeight(DAI);
            const mkrWeight = await kpool.getDenormalizedWeight(MKR);
            const wethWeight = await kpool.getDenormalizedWeight(WETH);
        ...
        it('allows adding token to the pool', async () => {
            ...
            const initialTokens = await kpool.getCurrentTokens();
            ...
            const tokens = await kpool.getCurrentTokens();

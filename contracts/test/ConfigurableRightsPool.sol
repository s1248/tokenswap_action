...
import "./PCToken.sol";
import "./KalancerReentrancyGuard.sol";
import "./KalancerOwnable.sol";
 ...
 * @notice PCToken is the "Kalancer Smart Pool" token (transferred upon finalization)
 ...
 *
 * Note that functions called on bPool and kFactory may look like internal calls,
 ...
 */
contract ConfigurableRightsPool is PCToken, KalancerOwnable, KalancerReentrancyGuard {
    using KalancerSafeMath for uint;
    using SafeApprove for IERC20;

    // Type declarations

    struct PoolParams {
        // Kiwi Pool Token (representing shares of the pool)
    ...
    PCToken(poolParams.poolTokenSymbol, poolParams.poolTokenName)
    {
        ...
        require(poolParams.swapFee >= KalancerConstants.MIN_FEE, "ERR_INVALID_SWAP_FEE");
        require(poolParams.swapFee <= KalancerConstants.MAX_FEE, "ERR_INVALID_SWAP_FEE");
        ...
        require(poolParams.constituentTokens.length >= KalancerConstants.MIN_ASSET_LIMIT, "ERR_TOO_FEW_TOKENS");
        require(poolParams.constituentTokens.length <= KalancerConstants.MAX_ASSET_LIMIT, "ERR_TOO_MANY_TOKENS");
        // There are further possible checks (e.g., if they use the same token twice), but
        // we can let bind() catch things like that (i.e., not things that might reasonably work)

        SmartPoolManager.verifyTokenCompliance(poolParams.constituentTokens);

        kFactory = IBFactory(factoryAddress);
        rights = rightsStruct;
        ...
        bspCap = KalancerConstants.MAX_UINT;
    }
    ...
    */
    ...
    lockUnderlyingPool
    {
        ...
        _pushPoolShare(address(kFactory), exitFee);
    ...
    returns (uint tokenAmountOut)
    {
        ...
        uint pAiAfterExitFee = KalancerSafeMath.bsub(poolAmountIn, exitFee);
    ...
    */
    ...
    /**
     * @notice Getter for the KalancerSafeMath contract
     * @dev Convenience function to get the address of the KalancerSafeMath library (so clients can check version)
     * @return address of the KalancerSafeMath library
    */
    function getKalancerSafeMathVersion() external pure returns (address) {
        return address(KalancerSafeMath);
    }
    ...
    */
    ...
    function createPoolInternal(uint initialSupply) internal {
        ...
        require(initialSupply >= KalancerConstants.MIN_POOL_SUPPLY, "ERR_INIT_SUPPLY_MIN");
        require(initialSupply <= KalancerConstants.MAX_POOL_SUPPLY, "ERR_INIT_SUPPLY_MAX");
        ...
        // Deploy new BPool (kFactory and bPool are interfaces; all calls are external)
        bPool = kFactory.newBPool();

        // EXIT_FEE must always be zero, or ConfigurableRightsPool._pushUnderlying will fail
        require(bPool.EXIT_FEE() == 0, "ERR_NONZERO_EXIT_FEE");
        require(KalancerConstants.EXIT_FEE == 0, "ERR_NONZERO_EXIT_FEE");
        ...
        for (uint i = 0; i < _initialTokens.length; i++) {
            ...
            returnValue = IERC20(t).safeApprove(address(bPool), KalancerConstants.MAX_UINT);
            require(returnValue, "ERR_ERC20_FALSE");
    ...
    // bPool is a contract interface; function calls on it are external
    function _pullUnderlying(address erc20, address from, uint amount) internal needsKPool {
        ...
        bPool.rebind(erc20, KalancerSafeMath.badd(tokenBalance, amount), tokenWeight);
    }

    // Rebind BPool and push tokens to address
    // bPool is a contract interface; function calls on it are external
    function _pushUnderlying(address erc20, address to, uint amount) internal needsKPool {
        // Gets current Balance of token i, Bi, and weight of token i, Wi, from BPool.
        uint tokenBalance = bPool.getBalance(erc20);
        uint tokenWeight = bPool.getDenormalizedWeight(erc20);
        bPool.rebind(erc20, KalancerSafeMath.bsub(tokenBalance, amount), tokenWeight);
